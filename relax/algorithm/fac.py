...
from relax.network.fac import FACNet, FACParams
from relax.utils.experience import Experience
from relax.utils.typing import Metric


class FACOptStates(NamedTuple):
    q1: optax.OptState
    q2: optax.OptState
...
class FACTrainState(NamedTuple):
    params: FACParams
    opt_state: FACOptStates
    step: int
    ...
    def __init__(
        ...
        *,
        ...
        def stateless_update(
            key: jax.random.KeyArray, state: FACTrainState, data: Experience
        ) -> Tuple[FACTrainState, Metric]:
            ...
            def policy_loss_fn(policy_params: hk.Params) -> jnp.ndarray:
                ...
                multiplier = self.agent.multiplier(multiplier_params, obs)
                policy_loss = jnp.mean(jnp.exp(log_alpha) * new_logp - q + multiplier * cost_q)
                return policy_loss, (q1, q2, cost_q, new_logp, multiplier)

            (policy_loss, aux), policy_grads = jax.value_and_grad(policy_loss_fn, has_aux=True)(policy_params)
            q1, q2, cost_q, new_logp, multiplier = aux
            ...
            # update multiplier
            def multiplier_loss_fn(multiplier_params: hk.Params) -> jnp.ndarray:
                multiplier = self.agent.multiplier(multiplier_params, obs)
                multiplier_loss = -jnp.mean(multiplier * cost_q)
                return multiplier_loss

            def multiplier_update_fn(
                multiplier_params: hk.Params, multiplier_opt_state: optax.OptState
            ) -> Tuple[jnp.ndarray, optax.OptState]:
                multiplier_grads = jax.grad(multiplier_loss_fn)(multiplier_params)
                multiplier_update, multiplier_opt_state = self.multiplier_optim.update(
                    multiplier_grads, multiplier_opt_state
                )
                multiplier_params = optax.apply_updates(multiplier_params, multiplier_update)
