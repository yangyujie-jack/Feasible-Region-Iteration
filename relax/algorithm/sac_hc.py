...
from relax.network.sac_hc import SACHCNet, SACHCParams
from relax.utils.experience import Experience
from relax.utils.typing import Metric


class SACHCOptStates(NamedTuple):
    q1: optax.OptState
    q2: optax.OptState
...
class SACHCTrainState(NamedTuple):
    params: SACHCParams
    opt_state: SACHCOptStates
    step: int


class SACHC(Algorithm):
    def __init__(
        self,
        agent: SACHCNet,
        params: SACHCParams,
        *,
        ...
        def stateless_update(
            key: jax.random.KeyArray, state: SACHCTrainState, data: Experience
        ) -> Tuple[SACHCTrainState, Metric]:
            ...
            def policy_loss_fn(policy_params: hk.Params):
                ...
                q = jnp.minimum(q1, q2)
                barrier = self.agent.barrier(obs_preprocess)
                new_next_obs = obs_preprocess + self.agent.model(model_params, obs_preprocess, new_action)
                next_barrier = self.agent.barrier(new_next_obs)
                barrier_penalty = jnp.maximum(next_barrier - (1 - self.lam) * barrier, 0) * self.penalty_scale
